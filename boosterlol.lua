...
local assistForce = 10 -- Default speed value
local moveForward = false
local moveBackward = false
local guiVisible = true -- Track GUI visibility
local boostEnabled = true -- Track boost state
local boostToggleKey = Enum.KeyCode.E -- Default keybind for boost toggle
local menuToggleKey = Enum.KeyCode.Insert -- Default keybind for menu toggle
...
local colorIndex = 1
local colorTransitionTime = 3 -- Seconds for one color transition
local elapsedTime = 0
...
-- Function to clamp frame position to screen
...
-- Input handling for GUI toggle, boost toggle, and movement
...
-- Function to set up the character and root references, and handle velocity boost
local heartbeatConnection = nil

local function setupCharacter(character)
    -- Disconnect any existing Heartbeat connection
    if heartbeatConnection then
        heartbeatConnection:Disconnect()
        heartbeatConnection = nil
    end

    -- Wait for the character model in workspace
